...
async function UpdateDetails(req, res) {
    ...
    const updateObject = {};
    if (name) updateObject.uname = name;
    if (email) updateObject.uemail = email;
    if (password) updateObject.upassword = password;
    if (profile) updateObject.profile = profile; // Include profile even if null
    updateObject.updatedAt = new Date();
  
    try {
      // Update user information in the database
      const [numAffectedRows, updatedUsers] = await userinfo.update(updateObject, { where: { uid: id } });
  
      if (numAffectedRows > 0) {
        // Fetch the updated user details
        const updatedUser = await userinfo.findOne({attributes: ['uname', 'uemail', 'profile'], where: { uid: id } });
